...
void Sprite::Init::enemy(Block::Colour * arr)
{
  ...
  for(std::size_t i = 0; i < 4; i++){
    arr[arr_loc(i+4,8)] = Block::Colour::RED;
    arr[arr_loc(i+4,9)] = Block::Colour::GREEN;
    arr[arr_loc(i+4,10)] = Block::Colour::RED;
  }
  for(std::size_t i = 0; i < 3; i++){
    arr[arr_loc(i+2,9-(i+2))] = Block::Colour::RED;
    arr[arr_loc(i+7,i+5)] = Block::Colour::RED;
    arr[arr_loc(i+1,8)] = Block::Colour::GREEN;
    arr[arr_loc(i+8,8)] = Block::Colour::GREEN;
  }
  for(std::size_t i = 5; i < 8; i++){
    arr[arr_loc(5,i)] = Block::Colour::BLACK;
    arr[arr_loc(6,i)] = Block::Colour::BLACK;
  }
  arr[arr_loc(3,7)] = Block::Colour::GREEN;
  arr[arr_loc(8,7)] = Block::Colour::GREEN;
  texture.loadFromImage(Block::to_image(arr));
  Sprite::textureMap[Block::Type::ENEMY_UP] = texture;
  //ENEMY_LEFT
  reset_col(arr);
  create_head_block(arr, 2,1);
  for(std::size_t i = 2; i < 5; i++){
    arr[arr_loc(i,3)] = Block::Colour::BLANK;
  }
  for(std::size_t i = 3; i < 9; i++){
    arr[arr_loc(i,i+2)] = Block::Colour::BLACK;
    arr[arr_loc(i+1,i+2)] = Block::Colour::BLACK;
  }
  for(std::size_t i = 4; i < 11; i++){
    arr[arr_loc(10,i)] = Block::Colour::BLACK;
  }
  for(std::size_t i = 3; i < 6; i++){
    arr[arr_loc(i+4,i)] = Block::Colour::RED;
  }
  ...
  for(std::size_t i = 5; i < 8; i++){
    arr[arr_loc(2,i)] = Block::Colour::RED;
  }
  arr[arr_loc(9,4)] = Block::Colour::BLACK;
  arr[arr_loc(3,2)] = Block::Colour::RED;
  arr[arr_loc(1,2)] = Block::Colour::GREEN;
  arr[arr_loc(7,5)] = Block::Colour::RED;
  arr[arr_loc(9,3)] = Block::Colour::RED;
  arr[arr_loc(8,3)] = Block::Colour::GREEN;
  arr[arr_loc(7,4)] = Block::Colour::GREEN;
  arr[arr_loc(8,5)] = Block::Colour::GREEN;
  texture.loadFromImage(Block::to_image(arr));
  Sprite::textureMap[Block::Type::ENEMY_LEFT] = texture;
  //ENEMY_DOWN
  reset_col(arr);
  create_head_block(arr, 4,4);
  for(std::size_t i = 0; i < 4; i++){
    arr[arr_loc(i+4,1)] = Block::Colour::RED;
    arr[arr_loc(i+4,2)] = Block::Colour::GREEN;
    arr[arr_loc(i+4,3)] = Block::Colour::RED;
  }
  for(std::size_t i = 0; i < 3; i++){
    arr[arr_loc(i+2,11-(i+2))] = Block::Colour::RED;
    arr[arr_loc(i+7,i+7)] = Block::Colour::RED;
    arr[arr_loc(i+1,10)] = Block::Colour::GREEN;
    arr[arr_loc(i+8,10)] = Block::Colour::GREEN;
    arr[arr_loc(5,8+i)] = Block::Colour::BLACK;
    arr[arr_loc(6,8+i)] = Block::Colour::BLACK;
  }
  arr[arr_loc(3,9)] = Block::Colour::GREEN;
  arr[arr_loc(8,9)] = Block::Colour::GREEN;
  arr[arr_loc(5,6)] = Block::Colour::GREEN;
  arr[arr_loc(6,6)] = Block::Colour::GREEN;
  arr[arr_loc(4,5)] = Block::Colour::RED;
  arr[arr_loc(7,5)] = Block::Colour::RED;
  texture.loadFromImage(Block::to_image(arr));
